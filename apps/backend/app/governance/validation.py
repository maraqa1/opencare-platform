...
def validation_matrix(repo_root: Path) -> list[ValidationItem]:
    ...
    return [
        ...
        ValidationItem("table_detail", "Table Detail shows schema, attributes, classification state, owner, consumers", "pass" if tables and tables[0].attributes and "AttributeTable" in components else "fail", "YAML-declared attributes render while dbt catalog remains explicit missing evidence"),
        ValidationItem("attribute_policy_trace", "Attribute detail shows policy/version/rule/evidence or Unknown", "pass" if "AttributeDetailPanel" in components and "Policy version" in components and "Matched rule" in components else "fail", "Attribute detail renders policy/version/rule/evidence or Unknown"),
