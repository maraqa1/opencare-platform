...
def ensure_decision_schema() -> None:
    with connect() as conn:
        conn.execute(
            ...
            alter table if exists decision.decision_queue add column if not exists source_item_id varchar(100);
            alter table if exists decision.decision_queue add column if not exists source_type varchar(50);
            ...
            alter table if exists decision.decision_queue add column if not exists recommended_action varchar(150);
            alter table if exists decision.decision_queue add column if not exists recommended_owner varchar(150);
            alter table if exists decision.decision_queue add column if not exists recommended_channel varchar(50);
            alter table if exists decision.decision_queue add column if not exists decision_reason text;
            alter table if exists decision.decision_queue add column if not exists risk_of_inaction text;
            alter table if exists decision.decision_queue add column if not exists approval_required boolean not null default false;
            alter table if exists decision.decision_queue add column if not exists approval_role varchar(100);
            alter table if exists decision.decision_queue add column if not exists auto_dispatch_eligible boolean not null default false;
            alter table if exists decision.decision_queue add column if not exists manual_action_required boolean not null default false;
            alter table if exists decision.decision_queue add column if not exists decision_score numeric(14,2);
            alter table if exists decision.decision_queue add column if not exists recoverability_probability numeric(8,4);
            alter table if exists decision.decision_queue add column if not exists confidence numeric(8,4);
            alter table if exists decision.decision_queue add column if not exists policy_weight numeric(8,4);
            alter table if exists decision.decision_queue add column if not exists confidence_weight numeric(8,4);
            alter table if exists decision.decision_queue add column if not exists due_pressure varchar(40);
            alter table if exists decision.decision_queue add column if not exists expected_effort_hours numeric(10,2);
            alter table if exists decision.decision_queue add column if not exists expected_roi_per_hour numeric(14,2);
            alter table if exists decision.decision_queue add column if not exists comparable_case_support jsonb not null default '[]';
            alter table if exists decision.decision_queue add column if not exists outcome_status varchar(20) not null default 'pending';
            alter table if exists decision.decision_queue add column if not exists generated_by varchar(100);
            alter table if exists decision.decision_queue add column if not exists approved_by varchar(100);
            alter table if exists decision.decision_queue add column if not exists approved_at timestamp;
            alter table if exists decision.decision_queue add column if not exists dispatched_at timestamp;
            alter table if exists decision.decision_queue add column if not exists closed_at timestamp;
