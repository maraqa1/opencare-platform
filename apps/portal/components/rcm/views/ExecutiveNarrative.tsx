    ...
    <div style={{ fontFamily: "var(--font-body)", color: "var(--oc-gray-900)" }}>
      ...
      {!loading && !error && !data?.meta?.empty && data && (
        <>
          ...
          {/* KPI grid from cash_impact */}
          <KPIGrid items={[
            { label: "RECOVERABLE · 7 DAYS",  value: currencyCompact(ci?.recoverable_cash_7d) },
            { label: "RECOVERABLE · 14 DAYS", value: currencyCompact(ci?.recoverable_cash_14d) },
            { label: "CASH AT RISK",          value: currencyCompact(ci?.cash_at_risk) },
            { label: "EXPECTED COLLECTIONS",  value: currencyCompact(ci?.expected_collections) },
          ]} />
              ...
              {(data.key_drivers ?? []).length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {(data.key_drivers ?? []).map((driver, i) => {
                    // Detect direction from leading ↑/↓ if present, else treat as neutral
                    const dir: "up" | "down" | "flat" =
                      driver.startsWith("↑") ? "up" :
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
            ...
            <div style={{ background: "rgba(255, 255, 255, 0.92)", border: "1px solid rgba(31, 56, 100, 0.08)", borderRadius: 20, padding: 22, boxShadow: "0 18px 40px rgba(31, 56, 100, 0.08)" }}>
              ...
              <MetricRow label="14-Day Recoverable" value={currencyCompact(ci?.recoverable_cash_14d)} target="—" good={true} />
              <MetricRow label="Expected Collections" value={currencyCompact(ci?.expected_collections)} target="—" good={true} />
            </div>
