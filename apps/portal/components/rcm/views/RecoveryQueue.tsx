        ...
        <>
              ...
              <tbody>
                {filteredAndScored.map((item, i) => {
                  ...
                  return (
                    ...
                    >
                      <td style={{ padding: "10px 12px", fontFamily: "monospace", fontSize: 11, color: "#1a3050" }}>
                        {ref}
                        {overdue && (
                          <span style={{ marginLeft: 4, padding: "1px 4px", borderRadius: 3, fontSize: 9, background: "#fdecea", color: "#c0392b", fontWeight: 600 }}>
                            OVERDUE
                          </span>
                        )}
                      </td>
                      ...
                      <td style={{ padding: "10px 12px", whiteSpace: "nowrap", maxWidth: 150, overflow: "hidden", textOverflow: "ellipsis" }}>
                        {ownerLabel(item.owner ?? item.owner_user_id ?? item.owner_team)}
                      </td>
                      <td style={{ padding: "10px 12px", whiteSpace: "nowrap", color: overdue ? "#c0392b" : "#1a1a1a" }}>
                        {shortDate(item.due_date)}
                      </td>
                      <td style={{ padding: "10px 12px" }}>
                        <StatusPill status={status} />
                      </td>
                      <td style={{ padding: "10px 12px", maxWidth: 180, fontSize: 11, color: "#555" }}>
                        {nextStep}
                      </td>
                    </tr>
                  );
                ...
                {filteredAndScored.length === 0 && (
                  <tr>
                    <td colSpan={10} style={{ padding: "24px", textAlign: "center", color: "#888", fontSize: 12 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10 }}>
            <p style={{ fontSize: 11, color: "#888", margin: 0 }}>
              Showing {filteredAndScored.length} of {data?.total ?? items.length} · Ranked by expected recovery per effort hour
            </p>
