export function currency(value?: number | null, currencyCode = "GBP") {
...
export function currencyCompact(value?: number | null, currencyCode = "GBP") {
  if (value == null) return "-";
  const prefix = currencyCode === "SAR" ? "SAR " : currencyCode === "GBP" ? "£" : `${currencyCode} `;
  if (Math.abs(value) >= 1_000_000) return `${prefix}${(value / 1_000_000).toFixed(1)}M`;
  if (Math.abs(value) >= 1_000) return `${prefix}${Math.round(value / 1_000)}K`;
