...
VALIDATE_DEMO_USE_CASES="${VALIDATE_DEMO_USE_CASES:-${DEMO_PROOF_FLOW_ENABLED:-false}}"
VALIDATE_DECISION_LAYER="${VALIDATE_DECISION_LAYER:-true}"
VALIDATE_INGESTION_LAYER="${VALIDATE_INGESTION_LAYER:-true}"
...
if [[ "$VALIDATE_DECISION_LAYER" == "true" ]]; then
  ...
  log_skip "Decision layer validation disabled"
fi

if [[ "$VALIDATE_DEMO_USE_CASES" == "true" ]]; then
...
\" | grep -qx t"
  ...
  log_skip "Demo/use-case validation disabled"
fi
...
if [[ "$VALIDATE_INGESTION_LAYER" == "true" ]] && [[ -f "$SCRIPT_DIR/../manifests/airbyte/values.template.yaml" ]]; then
...
elif [[ "$VALIDATE_INGESTION_LAYER" != "true" ]]; then
  log_skip "Ingestion validation disabled"
fi
