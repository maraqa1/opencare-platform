...
sync_dbt_project_configmap() {
  ...
  kubectl -n "$NAMESPACE" create configmap dbt-project-files \
    ...
    --from-file=fct_bed_pressure_distribution.sql="$ROOT_DIR/dbt/opencare/models/marts/fct_bed_pressure_distribution.sql" \
    --from-file=fct_bed_pressure_executive_actions.sql="$ROOT_DIR/dbt/opencare/models/marts/fct_bed_pressure_executive_actions.sql" \
