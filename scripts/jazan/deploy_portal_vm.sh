...
AI_GATEWAY_BASE_URL="${AI_GATEWAY_BASE_URL:-https://ai2.opendatalake.com/v1}"
LOCAL_AI_MODEL="${LOCAL_AI_MODEL:-mistral-nemo:12b}"
LOCAL_AI_REPORT_TIMEOUT_MS="${LOCAL_AI_REPORT_TIMEOUT_MS:-180000}"
LOCAL_AI_REPORT_SECTION_TIMEOUT_MS="${LOCAL_AI_REPORT_SECTION_TIMEOUT_MS:-25000}"
LOCAL_AI_REPORT_SECTION_CONCURRENCY="${LOCAL_AI_REPORT_SECTION_CONCURRENCY:-2}"
LOCAL_LLM_REPORT_MODE="${LOCAL_LLM_REPORT_MODE:-narrative_enrichment}"
LOCAL_LLM_MODEL="${LOCAL_LLM_MODEL:-mistral-nemo:12b}"
LOCAL_LLM_ENABLE_FIELD_ENRICHMENT="${LOCAL_LLM_ENABLE_FIELD_ENRICHMENT:-true}"
LOCAL_LLM_FIELD_TIMEOUT_MS="${LOCAL_LLM_FIELD_TIMEOUT_MS:-30000}"
LOCAL_LLM_ENRICHMENT_CONCURRENCY="${LOCAL_LLM_ENRICHMENT_CONCURRENCY:-1}"
LOCAL_LLM_MAX_FIELD_WORDS="${LOCAL_LLM_MAX_FIELD_WORDS:-120}"
LOCAL_LLM_MARKDOWN_REPORT_TIMEOUT_MS="${LOCAL_LLM_MARKDOWN_REPORT_TIMEOUT_MS:-180000}"
LOCAL_LLM_ENABLE_MARKDOWN_GENERATION="${LOCAL_LLM_ENABLE_MARKDOWN_GENERATION:-false}"
...
if "${KUBECTL[@]}" get configmap opencare-config >/dev/null 2>&1; then
  "${KUBECTL[@]}" patch configmap opencare-config --type merge \
    -p "{\"data\":{\"PORTAL_HOST\":\"${PUBLIC_PORTAL_HOST}\",\"PORTAL_URL\":\"${PUBLIC_PORTAL_URL}\",\"BASE_DOMAIN\":\"${PUBLIC_PORTAL_HOST}\",\"AI_GATEWAY_BASE_URL\":\"${AI_GATEWAY_BASE_URL}\",\"LOCAL_AI_MODEL\":\"${LOCAL_AI_MODEL}\",\"LOCAL_AI_REPORT_TIMEOUT_MS\":\"${LOCAL_AI_REPORT_TIMEOUT_MS}\",\"LOCAL_AI_REPORT_SECTION_TIMEOUT_MS\":\"${LOCAL_AI_REPORT_SECTION_TIMEOUT_MS}\",\"LOCAL_AI_REPORT_SECTION_CONCURRENCY\":\"${LOCAL_AI_REPORT_SECTION_CONCURRENCY}\",\"LOCAL_LLM_REPORT_MODE\":\"${LOCAL_LLM_REPORT_MODE}\",\"LOCAL_LLM_MODEL\":\"${LOCAL_LLM_MODEL}\",\"LOCAL_LLM_ENABLE_FIELD_ENRICHMENT\":\"${LOCAL_LLM_ENABLE_FIELD_ENRICHMENT}\",\"LOCAL_LLM_ENABLE_MARKDOWN_GENERATION\":\"${LOCAL_LLM_ENABLE_MARKDOWN_GENERATION}\",\"LOCAL_LLM_FIELD_TIMEOUT_MS\":\"${LOCAL_LLM_FIELD_TIMEOUT_MS}\",\"LOCAL_LLM_ENRICHMENT_CONCURRENCY\":\"${LOCAL_LLM_ENRICHMENT_CONCURRENCY}\",\"LOCAL_LLM_MAX_FIELD_WORDS\":\"${LOCAL_LLM_MAX_FIELD_WORDS}\",\"LOCAL_LLM_MARKDOWN_REPORT_TIMEOUT_MS\":\"${LOCAL_LLM_MARKDOWN_REPORT_TIMEOUT_MS}\"}}" >/dev/null
else
...
"${KUBECTL[@]}" set env "deployment/$DEPLOYMENT" "AI_GATEWAY_BASE_URL=${AI_GATEWAY_BASE_URL}" "LOCAL_AI_MODEL=${LOCAL_AI_MODEL}" "LOCAL_AI_REPORT_TIMEOUT_MS=${LOCAL_AI_REPORT_TIMEOUT_MS}" "LOCAL_AI_REPORT_SECTION_TIMEOUT_MS=${LOCAL_AI_REPORT_SECTION_TIMEOUT_MS}" "LOCAL_AI_REPORT_SECTION_CONCURRENCY=${LOCAL_AI_REPORT_SECTION_CONCURRENCY}" "LOCAL_LLM_REPORT_MODE=${LOCAL_LLM_REPORT_MODE}" "LOCAL_LLM_MODEL=${LOCAL_LLM_MODEL}" "LOCAL_LLM_ENABLE_FIELD_ENRICHMENT=${LOCAL_LLM_ENABLE_FIELD_ENRICHMENT}" "LOCAL_LLM_ENABLE_MARKDOWN_GENERATION=${LOCAL_LLM_ENABLE_MARKDOWN_GENERATION}" "LOCAL_LLM_FIELD_TIMEOUT_MS=${LOCAL_LLM_FIELD_TIMEOUT_MS}" "LOCAL_LLM_ENRICHMENT_CONCURRENCY=${LOCAL_LLM_ENRICHMENT_CONCURRENCY}" "LOCAL_LLM_MAX_FIELD_WORDS=${LOCAL_LLM_MAX_FIELD_WORDS}" "LOCAL_LLM_MARKDOWN_REPORT_TIMEOUT_MS=${LOCAL_LLM_MARKDOWN_REPORT_TIMEOUT_MS}" >/dev/null
"${KUBECTL[@]}" patch "deployment/$DEPLOYMENT" \
