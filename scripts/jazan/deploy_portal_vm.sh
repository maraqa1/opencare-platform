...
LOCAL_LLM_ENABLE_FIELD_ENRICHMENT="${LOCAL_LLM_ENABLE_FIELD_ENRICHMENT:-false}"
...
if "${KUBECTL[@]}" get configmap opencare-config >/dev/null 2>&1; then
  "${KUBECTL[@]}" patch configmap opencare-config --type merge \
    -p "{\"data\":{\"PORTAL_HOST\":\"${PUBLIC_PORTAL_HOST}\",\"PORTAL_URL\":\"${PUBLIC_PORTAL_URL}\",\"BASE_DOMAIN\":\"${PUBLIC_PORTAL_HOST}\",\"AI_GATEWAY_BASE_URL\":\"${AI_GATEWAY_BASE_URL}\",\"LOCAL_AI_MODEL\":\"${LOCAL_AI_MODEL}\",\"LOCAL_AI_REPORT_TIMEOUT_MS\":\"${LOCAL_AI_REPORT_TIMEOUT_MS}\",\"LOCAL_AI_REPORT_SECTION_TIMEOUT_MS\":\"${LOCAL_AI_REPORT_SECTION_TIMEOUT_MS}\",\"LOCAL_AI_REPORT_SECTION_CONCURRENCY\":\"${LOCAL_AI_REPORT_SECTION_CONCURRENCY}\",\"LOCAL_LLM_REPORT_MODE\":\"${LOCAL_LLM_REPORT_MODE}\",\"LOCAL_LLM_MODEL\":\"${LOCAL_LLM_MODEL}\",\"LOCAL_LLM_ENABLE_FIELD_ENRICHMENT\":\"${LOCAL_LLM_ENABLE_FIELD_ENRICHMENT}\",\"LOCAL_LLM_FIELD_TIMEOUT_MS\":\"${LOCAL_LLM_FIELD_TIMEOUT_MS}\",\"LOCAL_LLM_ENRICHMENT_CONCURRENCY\":\"${LOCAL_LLM_ENRICHMENT_CONCURRENCY}\",\"LOCAL_LLM_MAX_FIELD_WORDS\":\"${LOCAL_LLM_MAX_FIELD_WORDS}\",\"LOCAL_LLM_MARKDOWN_REPORT_TIMEOUT_MS\":\"${LOCAL_LLM_MARKDOWN_REPORT_TIMEOUT_MS}\"}}" >/dev/null
else
